...
module.exports = function(Sequelize) {
        ...
        afterFind: function(result, options) {
            ...
            if (options.hierarchy && options.includeMap) {
                ...
                if (include && include.where) {
                    console.log("Trying array" + util.inspect(include.where));
                    console.log("Trying array:" + this.hierarchy.throughForeignKey);
                    var foundParent = findObject(include.where, this.hierarchy.throughForeignKey);
                    if (foundParent) {
                        console.log("Found parrent in array");
                        parent = {};
                        parent[this.hierarchy.primaryKey] = foundParent;
                    }
                }
            }
    ...
    function findObject(theObject, target) {
        var result = null;
        if (theObject instanceof Array) {
            for (var i = 0; i < theObject.length; i++) {
                result = findObject(theObject[i], target);
                if (result) {
                    break;
                }
            }
            ...
            for (var prop in theObject) {
                console.log(prop + ': ' + theObject[prop] + '=>' + target);
                if (prop == target) {
                    return theObject[prop] 
                } 
                else {
                    result = findObject(theObject[prop], target);
                }
                if (result) {
                    break;
                }
